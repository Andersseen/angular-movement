...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 min-h-[calc(100vh-theme('spacing.64'))]">
      <div class="mb-12">
        ...
        <p class="text-lg text-text-muted max-w-2xl">
          Tweak the settings below to see how Angular Movement directs elements in and out of the DOM. The generated code updates in real-time.
        </p>
      </div>
      ...
      <div class="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
        ...
        <div class="w-full lg:w-[40%] bg-surface border border-border rounded-2xl p-6 md:p-8 shrink-0 shadow-sm">
          ...
          <div class="space-y-8">
            ...
            <div>
              ...
              <div class="relative">
                ...
                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-text-subtle">
              ...
              <div class="grid grid-cols-2 gap-2">
                @for (ease of ['ease', 'ease-in', 'ease-out', 'ease-in-out']; track ease) {
                  <button 
                    ...
                    [class]="easing() === ease ? 'bg-accent text-white border-accent' : 'bg-bg text-text-muted border-border hover:border-accent/50'"
        ...
        <div class="w-full lg:w-[60%] flex flex-col gap-6">
          ...
          <!-- Live Preview Area -->
          <div class="bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAAXNSR0IArs4c6QAAACVJREFUKFNjZCASMDKgAnv37v3PjE8RXxKYIsIymIagGgYKAAAACv0E/7zZ9k0AAAAASUVORK5CYII=')] bg-repeat border border-border rounded-2xl h-[320px] flex items-center justify-center relative overflow-hidden group">
            ...
            <div class="absolute bottom-4 right-4 z-20">
              <span class="text-xs font-mono text-text-subtle bg-bg/80 backdrop-blur px-2 py-1 rounded border border-border">Preview Area</span>
            </div>
          </div>

          <!-- Generated Code Snippet -->
          <div class="relative bg-code-bg rounded-2xl border border-border overflow-hidden">
            <div class="absolute top-0 left-0 right-0 h-10 border-b border-border bg-surface flex items-center px-4 justify-between z-20">
            ...
            <div class="pt-14 p-6 overflow-x-auto relative">
              <pre class="text-sm font-mono leading-relaxed"><code class="text-text">&lt;<span class="code-keyword">div</span>
      ...
      <div class="mt-24 border-t border-border pt-16">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          @for (p of allPresets; track p) {
            <button 
              (click)="setPreset(p)"
              [class]="preset() === p ? 'border-accent bg-accent/5' : 'border-border bg-surface hover:border-accent/40 hover:bg-surface-raised'"
              class="px-4 py-4 rounded-xl border transition-all text-center group"
            >
              <code class="text-sm font-mono transition-colors" [class]="preset() === p ? 'text-accent' : 'text-text-muted group-hover:text-text'">{{ p }}</code>
            </button>
          }
        </div>
      </div>
...
export class Demos {
  protected readonly allPresets: MovePreset[] = [
    ...
    'zoom-in', 'zoom-out', 'flip-x', 'flip-y', 'bounce-in'
  ];
