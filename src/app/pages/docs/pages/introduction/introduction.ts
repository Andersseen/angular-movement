    ...
    <article class="max-w-3xl">
      <div class="mb-10 pb-10 border-b border-border">
        ...
        <p class="text-xl text-text-muted mt-6">
          Angular Movement is an attribute-based animation library for Angular 21.
          Inspired by motion.dev and animate.style, but built natively for Angular.
        </p>
      </div>
      ...
      <div class="prose prose-invert prose-p:text-text-muted prose-headings:text-text prose-headings:font-display prose-a:text-accent prose-a:no-underline hover:prose-a:underline prose-code:text-accent-light prose-code:bg-surface-raised prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:before:content-none prose-code:after:content-none max-w-none">
        ...
        <p>
          Angular's native animation API (<code>@angular/animations</code>) is powerful but extremely verbose. 
          Writing triggers, transitions, and keyframes for simple enter/leave effects requires a lot of boilerplate and clutters your component decorator.
        </p>
        <p>
          Angular Movement wraps this complexity behind a single declarative HTML attribute:
        </p>
        
        <div class="my-6 bg-code-bg rounded-xl border border-border p-4 overflow-x-auto not-prose">
          <pre class="text-sm font-mono"><code class="text-text">&lt;<span class="code-keyword">div</span> <span class="code-attr">moveEnter</span>=<span class="code-string">"fade-up"</span>&gt;This fades up on enter&lt;/<span class="code-keyword">div</span>&gt;</code></pre>
        </div>

        <h2>Key features</h2>
        <ul class="space-y-2 mt-4">
          <li class="flex items-start gap-3">
            <svg class="w-5 h-5 text-accent shrink-0 mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
      ...
      <div class="mt-16 pt-8 border-t border-border flex justify-end">
        <a href="/docs/get-started" class="group flex flex-col items-end gap-1 hover:text-accent transition-colors">
