  ...
  template: `
    <article class="max-w-3xl">
      ...
      <div class="prose prose-invert prose-p:text-text-muted prose-headings:text-text prose-headings:font-display prose-a:text-accent prose-code:text-accent-light prose-code:bg-surface-raised max-w-none">
        ...
        <div class="my-6 bg-code-bg rounded-xl border border-border p-4 overflow-x-auto not-prose relative group">
          ...
          <pre class="text-sm font-mono mt-8"><code class="text-text"><span class="code-keyword">npm</span> install @angular-movement/core</code></pre>
        </div>

        <h2>2. Register the Provider</h2>
        <p>In your <code>app.config.ts</code>, provide the Movement configuration. You can optionally pass a global config object to override default durations or easings.</p>
        
        <div class="my-6 bg-code-bg rounded-xl border border-border p-4 overflow-x-auto not-prose">
          <pre class="text-sm font-mono"><code class="text-text"><span class="code-comment">// app.config.ts</span>
...
{{ '}' }};</code></pre>
        ...
        <p>Import <code>MOVEMENT_DIRECTIVES</code> into the components where you want to use the animations.</p>

        <div class="my-6 bg-code-bg rounded-xl border border-border p-4 overflow-x-auto not-prose">
          <pre class="text-sm font-mono"><code class="text-text"><span class="code-comment">// my.component.ts</span>
...
<span class="code-keyword">export class</span> MyComponent {{ '{' }}{{ '}' }}</code></pre>
        ...
        <p class="mt-8 p-4 bg-accent/5 border border-accent/20 rounded-xl">
          <strong>That's it!</strong> No triggers to set up, no state variables to manage. The element will animate automatically when initialized by Angular.
        </p>

      </div>
      
      <div class="mt-16 pt-8 border-t border-border flex justify-between items-center">
        <a href="/docs/introduction" class="group flex flex-col items-start gap-1 hover:text-accent transition-colors">
        ...
        <a href="/demos" class="group flex flex-col items-end gap-1 hover:text-accent transition-colors">
